...
class JettyServer(resourceConfig: ResourceConfig, servlets: Map<Class<out Servlet>, String> = emptyMap()) {
    ...
    init {
        (LoggerFactory.getLogger(Logger.ROOT_LOGGER_NAME) as Logger).level = Level.toLevel(System.getProperty("org.promethist.common.server.logLevel", "INFO"))
        val server = Server(AppConfig.instance.get("server.port", System.getProperty("org.promethist.common.server.port", "8080")).toInt())
