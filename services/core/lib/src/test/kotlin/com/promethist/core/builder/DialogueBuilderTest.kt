    ...
    fun `test dialogue building`() {
        dialogueBuilder.create("dialogue1", "product/dialogue/1").apply {
            source.apply {
                ...
                var nodeId = 0
                addNode(Speech(--nodeId, "response1", true, listOf("hello, say some animal", "hi, say some animal")))
                ...
                addNode(Speech(--nodeId, "response2", true, listOf("Your response was \${input.transcript.text}. Intent node \${input.intent.name}.")))
                addNode(UserInput(--nodeId, "input1", listOf("intent1", "intent2"), listOf(),false, mapOf(), ""))
